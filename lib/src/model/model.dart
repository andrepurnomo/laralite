...
abstract class Model<T extends Model<T>> {
    ...
    for (final entry in _fields.entries) {
      ...
      if (field.value != null) {
        ...
        if (serializedValue == 'NULL') {
          ...
          map[columnName] = serializedValue.substring(1, serializedValue.length - 1);
        } else {
  ...
  static Future<TModel?> find<TModel extends Model<TModel>>(dynamic id, TModel Function() constructor) async {
  ...
  static Future<List<TModel>> all<TModel extends Model<TModel>>(TModel Function() constructor) async {
    final instance = constructor();
  ...
  static Future<TModel> create<TModel extends Model<TModel>>(TModel model) async {
  ...
  static QueryBuilder<TModel> query<TModel extends Model<TModel>>(TModel Function() constructor) {
    final queryBuilder = QueryBuilder<TModel>(constructor);
    final instance = constructor();
  ...
  /// Paginate models
  static Future<PaginationResult<TModel>> paginate<TModel extends Model<TModel>>(
    TModel Function() constructor, {
    int page = 1,
    int perPage = 15,
  }) async {
    return await query<TModel>(constructor).paginate(page: page, perPage: perPage);
  }

  /// Find a model by primary key or throw exception
  static Future<TModel> findOrFail<TModel extends Model<TModel>>(dynamic id, TModel Function() constructor) async {
  ...
  static Future<List<TModel>> findMany<TModel extends Model<TModel>>(List<dynamic> ids, TModel Function() constructor) async {
    if (ids.isEmpty) return [];
    return await whereIn<TModel>(constructor, constructor().primaryKey, ids).get();
  }
  ...
  Future<bool> upsert({
    ...
    final updateMap = updateData ?? Map.fromEntries(
      map.entries.where((entry) => !conflictColumns.contains(entry.key))
    );
    
    final updateClause = updateMap.keys.map((key) => '$key = excluded.$key').join(', ');
  ...
  String _buildUpdateClause(Map<String, dynamic> map) {
    return map.keys.where((key) => key != primaryKey).map((key) => '$key = ?').join(', ');
  }
  
  List<dynamic> _buildUpdateValues(Map<String, dynamic> map) {
    return map.entries.where((entry) => entry.key != primaryKey).map((entry) => entry.value).toList();
  }
  
  String _buildInsertColumns(Map<String, dynamic> map) {
    return map.keys.where((key) => key != primaryKey || map[key] != null).join(', ');
  }
  
  String _buildInsertPlaceholders(Map<String, dynamic> map) {
    return map.keys.where((key) => key != primaryKey || map[key] != null).map((_) => '?').join(', ');
  }
  
  List<dynamic> _buildInsertValues(Map<String, dynamic> map) {
    return map.entries.where((entry) => entry.key != primaryKey || entry.value != null).map((entry) => entry.value).toList();
  }
  ...
  HasOne<TRelated> hasOne<TRelated extends Model<TRelated>>(
    ...
    return HasOne<TRelated>(
      ...
      foreignKey: foreignKey ?? '${table.substring(0, table.length - 1)}_id', // e.g., user_id from users
  ...
  BelongsTo<TRelated> belongsTo<TRelated extends Model<TRelated>>(
    ...
    return BelongsTo<TRelated>(
      parent: this,
      relatedConstructor: relatedConstructor,
      foreignKey: foreignKey ?? '${related.table.substring(0, related.table.length - 1)}_id', // e.g., user_id for users
  ...
  BelongsToMany<TRelated> belongsToMany<TRelated extends Model<TRelated>>(
    ...
    final relatedSingular = related.table.substring(0, related.table.length - 1);
