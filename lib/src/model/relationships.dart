...
class RelationshipRegistry {
  ...
  Future<T?> belongsTo<T extends Model<T>>(String name) => getBelongsTo<T>(name);
  
  /// Get BelongsToMany relationship (short alias)
  Future<List<T>> belongsToMany<T extends Model<T>>(String name) => getBelongsToMany<T>(name);
  
  /// Check if a relationship is registered
  bool has(String name) {
    return _relationships.containsKey(name) || _lazyRelationships.containsKey(name);
  }
