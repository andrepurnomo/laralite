...
class ValidationRegistry {
  ...
  bool hasFieldRules(String fieldName) {
    return _fieldRules.containsKey(fieldName) &&
        _fieldRules[fieldName]!.isNotEmpty;
  }
...
mixin ValidationMixin<T extends Model<T>> on Model<T> {
  ...
  void registerFieldValidation(
    String fieldName,
    List<ModelValidationRule> rules,
  ) {
...
class ModelValidationException implements Exception {
  ...
  @override
  String toString() =>
      'ModelValidationException: ${result.firstError ?? 'Validation failed'}';
}
