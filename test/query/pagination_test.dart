  ...
  group('Pagination Tests', () {
    setUp(() async {
      ...
      for (int i = 1; i <= 25; i++) {
        final article = Article()
          ...
          ..published =
              i % 3 ==
              0 // Every 3rd article is published
          ..views = i * 10;
        await article.save();
      }
    });
    ...
    group('Basic Pagination Tests', () {
      ...
      test('should paginate with custom per page', () async {
        final result = await Model.query<Article>(
          () => Article(),
        ).paginate(perPage: 10);
    ...
    group('Pagination with WHERE Conditions', () {
      test('should paginate filtered results', () async {
        final result = await Model.query<Article>(
          () => Article(),
        ).where('published', true).paginate(perPage: 5);
      ...
      test('should paginate with multiple conditions', () async {
        final result = await Model.query<Article>(
          () => Article(),
        ).where('published', true).where('views', '>', 50).paginate(perPage: 3);
    ...
    group('Pagination with ORDER BY', () {
      test('should paginate with ORDER BY ASC', () async {
        final result = await Model.query<Article>(
          () => Article(),
        ).orderBy('views').paginate(perPage: 5);
        ...
        for (int i = 0; i < result.data.length - 1; i++) {
          expect(
            result.data[i].views!,
            lessThanOrEqualTo(result.data[i + 1].views!),
          );
        }
      });
      ...
      test('should paginate with ORDER BY DESC', () async {
        final result = await Model.query<Article>(
          () => Article(),
        ).orderByDesc('views').paginate(perPage: 5);
        ...
        for (int i = 0; i < result.data.length - 1; i++) {
          expect(
            result.data[i].views!,
            greaterThanOrEqualTo(result.data[i + 1].views!),
          );
        }
      });

      test('should maintain order across pages', () async {
        final page1 = await Model.query<Article>(
          () => Article(),
        ).orderByDesc('views').paginate(page: 1, perPage: 5);

        final page2 = await Model.query<Article>(
          () => Article(),
        ).orderByDesc('views').paginate(page: 2, perPage: 5);

        // Last item of page 1 should have higher or equal views than first item of page 2
        expect(
          page1.data.last.views!,
          greaterThanOrEqualTo(page2.data.first.views!),
        );
      });
    });
    ...
    group('Complex Pagination Scenarios', () {
      test('should work with scopes', () async {
        ...
        final result = await Model.query<Article>(
          () => Article(),
        ).where('published', true).orderByDesc('views').paginate(perPage: 3);
      ...
      test('should work with LIMIT and OFFSET manually', () async {
        ...
        final manualResult = await Model.query<Article>(
          () => Article(),
        ).limit(10).offset(10).get();

        final paginatedResult = await Model.query<Article>(
          () => Article(),
        ).paginate(page: 2, perPage: 10);
