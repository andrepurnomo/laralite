  ...
  group('QueryBuilder Tests', () {
    ...
    group('WHERE Conditions', () {
      ...
      test('should combine different WHERE conditions', () {
        ...
        final sql = query.toSql();
        expect(sql, equals('SELECT * FROM users WHERE active = ? AND email IS NOT NULL AND age BETWEEN ? AND ?'));
    ...
    group('ORDER BY Clauses', () {
      ...
      test('should chain multiple ORDER BY clauses', () {
        ...
        final sql = query.toSql();
        expect(sql, equals('SELECT * FROM users ORDER BY active DESC, name ASC'));
      });
      
      test('should handle mixed ASC/DESC ordering', () {
        final query = QueryBuilder<TestUser>(() => TestUser());
        query.orderBy('active', 'DESC').orderBy('age', 'ASC').orderBy('name', 'DESC');
        final sql = query.toSql();
        expect(sql, equals('SELECT * FROM users ORDER BY active DESC, age ASC, name DESC'));
      });
    });
    ...
    group('Complex Query Building', () {
      test('should build complex query with all clauses', () {
        ...
        final sql = query.toSql();
        final expected = 'SELECT name, email, age FROM users WHERE active = ? AND email IS NOT NULL ORDER BY age DESC LIMIT 10 OFFSET 5';
    ...
    group('Advanced Features', () {
      test('should apply scope callback', () {
        ...
        expect(result, same(query));
        final sql = query.toSql();
        expect(sql, equals('SELECT * FROM users WHERE active = ? ORDER BY name ASC'));
      });
      ...
      test('should chain scope and when methods', () {
        ...
        final sql = query.toSql();
        expect(sql, equals('SELECT * FROM users WHERE active = ? AND age > ? ORDER BY name ASC'));
      });
    });
    ...
    group('OR Conditions Tests', () {
      ...
      test('should add OR WHERE IN condition', () {
        ...
        final sql = query.toSql();
        expect(sql, equals('SELECT * FROM users WHERE active = ? OR age IN (?, ?)'));
      ...
      test('should add OR WHERE NOT IN condition', () {
        ...
        final sql = query.toSql();
        expect(sql, equals('SELECT * FROM users WHERE active = ? OR age NOT IN (?, ?)'));
        expect(query.getBindings(), equals([1, 25, 30]));
      });
      ...
      test('should add OR WHERE NULL condition', () {
        ...
        final sql = query.toSql();
        expect(sql, equals('SELECT * FROM users WHERE active = ? OR email IS NULL'));
        expect(query.getBindings(), equals([1]));
      });
      ...
      test('should add OR WHERE NOT NULL condition', () {
        ...
        final sql = query.toSql();
        expect(sql, equals('SELECT * FROM users WHERE active = ? OR email IS NOT NULL'));
        expect(query.getBindings(), equals([1]));
      });
      ...
      test('should add OR WHERE BETWEEN condition', () {
        ...
        final sql = query.toSql();
        expect(sql, equals('SELECT * FROM users WHERE active = ? OR age BETWEEN ? AND ?'));
      ...
      test('should chain multiple OR conditions', () async {
        ...
        expect(users, hasLength(2)); // Bob Wilson (inactive) + Charlie Green (null email, age 45)
      });
    });
    ...
    group('Aggregation Methods Tests', () {
      ...
      test('should handle aggregation with WHERE conditions', () async {
        ...
        expect(sum, equals(128.0)); // 30 + 25 + 28 + 45 (excluding Bob Wilson who is inactive)
      });
    ...
    group('Edge Cases and Error Handling', () {
      ...
      test('should handle SELECT with empty column list', () {
        ...
        final sql = query.toSql();
        expect(sql, equals('SELECT  FROM users')); // May need handling in implementation
      });
