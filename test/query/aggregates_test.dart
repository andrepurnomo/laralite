...
void main() async {
  ...
  group('Aggregate Functions Tests', () {
    setUp(() async {
      ...
      // Insert test data
      final sales = [
        Sale()..productName = 'Laptop'..amount = 999.99..quantity = 1..categoryId = 1,
        Sale()..productName = 'Mouse'..amount = 25.50..quantity = 5..categoryId = 1,
        Sale()..productName = 'Keyboard'..amount = 75.00..quantity = 3..categoryId = 1,
        Sale()..productName = 'Monitor'..amount = 299.99..quantity = 2..categoryId = 2,
        Sale()..productName = 'Speaker'..amount = 150.00..quantity = 4..categoryId = 2,
      ];
    ...
    group('Count Aggregate Tests', () {
      ...
      test('should count with WHERE condition', () async {
        final category1Count = await Model.query<Sale>(() => Sale())
            .where('category_id', 1)
            .count();
      ...
      test('should count with multiple conditions', () async {
        final expensiveCategory1 = await Model.query<Sale>(() => Sale())
            .where('category_id', 1)
            .where('amount', '>', 50.0)
            .count();
      ...
      test('should return 0 for no matches', () async {
        final noMatches = await Model.query<Sale>(() => Sale())
    ...
    group('Sum Aggregate Tests', () {
      ...
      test('should sum with WHERE condition', () async {
        final category1Sum = await Model.query<Sale>(() => Sale())
            .where('category_id', 1)
            .sum('amount');
      ...
      test('should sum quantities', () async {
        final totalQuantity = await Model.query<Sale>(() => Sale()).sum('quantity');
    ...
    group('Average Aggregate Tests', () {
      ...
      test('should calculate average with WHERE condition', () async {
        final category2Avg = await Model.query<Sale>(() => Sale())
            .where('category_id', 2)
            .avg('amount');
      ...
      test('should calculate average quantity', () async {
        final avgQuantity = await Model.query<Sale>(() => Sale()).avg('quantity');
    ...
    group('Min Aggregate Tests', () {
      ...
      test('should find minimum with WHERE condition', () async {
        final category1Min = await Model.query<Sale>(() => Sale())
            .where('category_id', 1)
            .min('amount');
      ...
      test('should find minimum quantity', () async {
        final minQuantity = await Model.query<Sale>(() => Sale()).min('quantity');
    ...
    group('Max Aggregate Tests', () {
      ...
      test('should find maximum with WHERE condition', () async {
        final category2Max = await Model.query<Sale>(() => Sale())
            .where('category_id', 2)
            .max('amount');
      ...
      test('should find maximum quantity', () async {
        final maxQuantity = await Model.query<Sale>(() => Sale()).max('quantity');
    ...
    group('Complex Aggregate Queries', () {
      test('should combine aggregates with GROUP BY-like logic', () async {
        ...
        final cat1Query = Model.query<Sale>(() => Sale()).where('category_id', 1);
        final cat1Count = await cat1Query.count();
        final cat1Sum = await Model.query<Sale>(() => Sale()).where('category_id', 1).sum('amount');
        final cat1Avg = await Model.query<Sale>(() => Sale()).where('category_id', 1).avg('amount');
      ...
      test('should work with multiple WHERE conditions', () async {
        final complexQuery = Model.query<Sale>(() => Sale())
            .where('category_id', 1)
            .where('quantity', '>=', 3);
            
        final count = await complexQuery.count();
        final sum = await Model.query<Sale>(() => Sale())
            .where('category_id', 1)
            .where('quantity', '>=', 3)
            .sum('amount');
      ...
      test('should work with ORDER BY and LIMIT', () async {
        ...
        final topExpensiveCount = await Model.query<Sale>(() => Sale())
            .where('amount', '>', 100.0)
            .orderByDesc('amount')
            .limit(2)
            .count();
