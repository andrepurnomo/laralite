...
class TestMutatorUser extends Model<TestMutatorUser> with MutatorAccessorMixin<TestMutatorUser> {
  ...
  final _price = IntField(); // Changed to IntField since DollarsToCentsMutator outputs int
  ...
  void initializeMutatorAccessors() {
    ...
    registerAccessor('credit_card', MaskAccessor(visibleStart: 0, visibleEnd: 4));
  ...
  group('Mutators and Accessors System Tests', () {
    ...
    group('Built-in Mutators Tests', () {
      test('PasswordMutator should hash passwords', () {
        ...
        expect(result1, isNot(equals(result3))); // Different input should give different hash
      ...
      test('EmailMutator should normalize email addresses', () {
        ...
        expect(mutator.mutate('  user@example.com  '), equals('user@example.com'));
        expect(mutator.mutate('User.Name@Domain.Com'), equals('user.name@domain.com'));
        expect(mutator.mutate('test+label@sub.domain.org'), equals('test+label@sub.domain.org'));
      });
      ...
      test('CapitalizeMutator should capitalize words', () {
        ...
        expect(mutator.mutate('  spaced  words  '), equals('  Spaced  Words  '));
    ...
    group('Built-in Accessors Tests', () {
      ...
      test('FullNameAccessor should combine names', () {
        ...
        expect(accessor.access({'first': 'John', 'last': 'Doe'}), equals('John Doe'));
        expect(accessor.access({'first': 'Mary', 'last': null}), equals('Mary'));
        expect(accessor.access({'first': null, 'last': 'Smith'}), equals('Smith'));
        expect(accessor.access({'first': null, 'last': null}), equals(''));
        expect(accessor.access({'first': '', 'last': ''}), equals(''));
        expect(accessor.access({'first': 'Madonna', 'last': ''}), equals('Madonna'));
      });
      ...
      test('InitialsAccessor should extract initials', () {
        ...
        expect(accessor.access({'first': 'mary', 'last': 'jane'}), equals('MJ'));
      ...
      test('MaskAccessor should mask sensitive data', () {
        ...
        expect(defaultAccessor.access('1234567890123456'), equals('************3456'));
        expect(defaultAccessor.access('abcd'), equals('abcd')); // Too short to mask (4 chars = visibleStart 0 + visibleEnd 4)
        expect(defaultAccessor.access('1234'), equals('1234')); // Exactly at threshold
        
        final customAccessor = MaskAccessor(visibleStart: 2, visibleEnd: 2, maskChar: '#');
    ...
    group('MutatorAccessorRegistry Tests', () {
      ...
      test('should apply mutators to values', () {
        ...
        expect(registry.applyMutator('email', 'USER@EXAMPLE.COM'), equals('user@example.com'));
        expect(registry.applyMutator('name', 'John Doe'), equals('John Doe')); // No mutator
      ...
      test('should apply accessors to values', () {
        ...
        expect(registry.applyAccessor('name', 'John Doe'), equals('John Doe')); // No accessor
    ...
    group('MutatorAccessorMixin Integration Tests', () {
      ...
      test('should register additional mutators and accessors dynamically', () {
        ...
        expect(user.getValueWithAccessor<String>('first_name'), equals('******Name'));
      });
