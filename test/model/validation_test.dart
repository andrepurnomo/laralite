...
class TestValidationUser extends Model<TestValidationUser> with ValidationMixin<TestValidationUser> {
  ...
  void initializeValidation() {
    ...
    registerFieldValidation('email', [
      RequiredRule<String>(),
      EmailRule(),
    ]);
    
    registerFieldValidation('age', [
      RangeRule<int>(min: 0, max: 120),
    ]);
    
    registerFieldValidation('phone', [
      RegexRule(RegExp(r'^\+?[\d\s\-\(\)]+$'), customMessage: 'Invalid phone format'),
    ]);
    
    registerFieldValidation('password', [
      RequiredRule<String>(),
      MinLengthRule(8),
      RegexRule(RegExp(r'^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)'), customMessage: 'Password must contain uppercase, lowercase and digit'),
    ]);
    ...
    registerFieldValidation('score', [
      RangeRule<double>(min: 0.0, max: 100.0),
    ]);
  ...
  group('Validation System Tests', () {
    ...
    group('ModelValidationResult Tests', () {
      ...
      test('should create invalid result with field errors', () {
        ...
        expect(result.getFirstFieldError('email'), equals('Invalid email format'));
      ...
      test('should combine multiple validation results', () {
        ...
        final result2 = ModelValidationResult(
          isValid: false,
          fieldErrors: {'email': ['Invalid'], 'name': ['Too short']},
      ...
      test('should handle toString formatting', () {
        ...
        expect(invalidResult.toString(), contains('ValidationResult(invalid, 2 errors:'));
      });
    });
    ...
    group('Validation Rules Tests', () {
      ...
      test('MinLengthRule should validate correctly', () {
        ...
        expect(rule.getMessageFor('password'), equals('password must be at least 5 characters'));
      });
      ...
      test('MaxLengthRule should validate correctly', () {
        ...
        expect(rule.getMessageFor('name'), equals('name must be no more than 10 characters'));
      });
      ...
      test('EmailRule should validate correctly', () {
        ...
        expect(rule.getMessageFor('email'), equals('email must be a valid email address'));
      });
      ...
      test('RangeRule should validate numbers correctly', () {
        ...
        expect(rule.getMessageFor('age'), equals('age must be between 10 and 100'));
      ...
      test('RegexRule should validate patterns correctly', () {
        final phoneRule = RegexRule(RegExp(r'^\+?[\d\s\-\(\)]+$'), customMessage: 'Invalid phone');
    ...
    group('ValidationRegistry Tests', () {
      ...
      test('should register and retrieve model rules', () {
        ...
        registry.addModelRule(CustomRule<dynamic>((model) => false, customMessage: 'Second rule'));
      ...
      test('should clear all rules', () {
        ...
        registry.addFieldRule('name', RequiredRule<String>());
        registry.addModelRule(CustomRule<dynamic>((model) => true, customMessage: 'Test'));
    ...
    group('ValidationMixin Tests', () {
      ...
      test('should add validation rules dynamically', () {
        ...
        user.addFieldValidation('name', 
          RegexRule(RegExp(r'^[A-Z]'), customMessage: 'Name must start with uppercase')
        );
        ...
        final result = user.validateModel();
        expect(result.isValid, isFalse);
        expect(result.getFieldErrors('name'), contains('Name must start with uppercase'));
      });
    });
    ...
    group('ModelValidationException Tests', () {
      ...
      test('should handle empty error lists', () {
        ...
        expect(exception.toString(), equals('ModelValidationException: Validation failed'));
      });
    });
