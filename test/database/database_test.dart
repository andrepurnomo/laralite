...
void main() {
  group('Database Connection Tests', () {
    ...
    test(
      'should throw error when using database before initialization',
      () async {
    ...
    test('should insert and query data with parameters', () async {
      ...
      final users = await Database.query('SELECT * FROM users WHERE name = ?', [
        'John Doe',
      ]);
    ...
    test('should handle multiple inserts and updates', () async {
      ...
      // Insert initial data
      await Database.execute('INSERT INTO counters (id, value) VALUES (?, ?)', [
        1,
        10,
      ]);
  ...
  group('Database Connection Tests', () {
    ...
    test(
      'should maintain separate database instances for different paths',
      () async {
  ...
  group('Database Isolate Tests', () {
    ...
    tearDown(() async {
      ...
      ); // Longer delay for isolate cleanup
    });
    ...
    test('should handle multiple rapid requests to isolate', () async {
      ...
      for (int i = 0; i < 20; i++) {
        futures.add(
          Database.execute('INSERT INTO rapid_test (value) VALUES (?)', [i]),
        );
      }
      ...
      final count = await Database.query(
        'SELECT COUNT(*) as count FROM rapid_test',
      );
    ...
    test('should handle concurrent mixed operations (query/execute)', () async {
      ...
      for (int i = 0; i < 5; i++) {
        futures.add(
          ...
          ), // Simple query not dependent on table contents
        );
      }
      ...
      final finalCount = await Database.query(
  ...
  group('Database Error Handling Tests', () {
    ...
    test('should handle invalid SQL syntax gracefully', () async {
      ...
      // Invalid INSERT
      expect(() => Database.execute('INSERT WRONG SYNTAX'), throwsException);
  ...
  group('Database Performance Tests', () {
    ...
    test(
      'should handle memory usage efficiently with large datasets',
      () async {
        await Database.initialize(databasePath: ':memory:');
